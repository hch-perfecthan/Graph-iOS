...
internal extension SwiftScrollViewExtension where Self:UIScrollView{
   ...
   /*internal*/ func addNotifications(){
    ...
    /**
     * PERFECTHAN : 다음 UITextField 객체를 가져오는 메소드 추가
     */
    func next(to textField: UITextField) -> UITextField? {
        if let nextComponent = textField.nextTextComponent {
            if nextComponent is UITextField {
                return nextComponent as? UITextField;
            }
            if nextComponent is UITextView {
    ...
    /**
     * PERFECTHAN : 다음 UITextField로 이동하는 메소드 추가
     */
    func next(for textField: UITextField) {
        if let nextComponent = textField.nextTextComponent {
            nextComponent.becomeFirstResponder()
            if nextComponent is UITextView {
                self.setContentForTextComponent()
